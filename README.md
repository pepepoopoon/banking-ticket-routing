...
Параметры и сводка 104 выполненных запусков приведены в
...
На синтетическом baseline test macro-F1 и top-3 accuracy равны `1.0000`, test coverage —
`0.6000`. При 40% шума train-меток macro-F1 снизился до `0.4806`, а top-3 accuracy —
до `0.8000`. Эти числа характеризуют только контролируемые проверки pipeline; метрики
на BANKING77 или реальных обращениях без отдельной оценки не заявляются.
